...
* Go to `tools` folder and run `python extract_frames.py --root_path ../data/Custom/data/$SUBJECT_ID` to extract frames. For the avatar creation, we recommend downsampling the video to 5 fps to remove redundancy and make the pipeline faster.
